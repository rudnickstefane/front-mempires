...
export const useVisitAlterForm = ({
    ...
    const [referralSelected, setReferralSelected] = useState<string>(data.referralSource);
    ...
    const [formData, setFormData] = useState<VisitRegisterProps['formData']>({
        ...
        modalities: data.categories?.map((item: { categoryCode: string; }) => String(item.categoryCode)),
    ...
    const validateForm = () => {
        ...
        const indicationCode = responseStudent?.findStudent.profileCode ? responseStudent.findStudent.profileCode : data.indicationCode;
