...
export const useReviewUpsertForm = ({
    ...
    const handleTextFieldChange = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (formData.protocol === 'JPW78807') {
            ...
            if (data.gender === 'MAN') {
                const density = 1.112 - 0.00043499 * sd + 0.00000055 * Math.pow(sd, 2) - 0.00028826 * age;
                ...
                const density = 1.097 - 0.00046971 * sd + 0.00000056 * Math.pow(sd, 2) - 0.00012828 * age;
                ...
                const taxametabólicabasal = 10 * weight + 6.25 * heightCm - 5 * age - 161;
                ...
                const densityMan = 1.112 - 0.00043499 * sd + 0.00000055 * Math.pow(sd, 2) - 0.00028826 * age;
                const densityWoman = 1.097 - 0.00046971 * sd + 0.00000056 * Math.pow(sd, 2) - 0.00012828 * age;
        ...
        if (formData.protocol === 'JPW78803') {
            ...
            if (data.gender === 'MAN') {
                const density = 1.10938 - 0.0008267 * sd + 0.0000016 * Math.pow(sd, 2) - 0.0002574 * age;
                ...
                const density = 1.0994921 - 0.0009929 * sd + 0.0000023 * Math.pow(sd, 2) - 0.0001392 * age;
                ...
                const densityMan = 1.10938 - 0.0008267 * sd + 0.0000016 * Math.pow(sd, 2) - 0.0002574 * age;
                const densityWoman = 1.0994921 - 0.0009929 * sd + 0.0000023 * Math.pow(sd, 2) - 0.0001392 * age;
                const densityAverage = (densityMan + densityWoman) / 2;
                const fatPercentage = (4.95 / densityAverage - 4.5) * 100;
        ...
        if (formData.protocol === 'P954') {
            ...
            const density = 1.10726863 - 0.00081201 * sd + 0.00000212 * Math.pow(sd, 2) - 0.00041761 * age;
        ...
        if (formData.protocolCardio === 'M1609') {
            ...
            const vo = 132.853 - (0.0769 * weight / 0.454) - (0.3877 * age) + (6.315 * (data.gender === 'MAN' ? 1 : 0)) - (3.2649 * t) - (0.1565 * fc);
    ...
    const handleFinish = async () => {
        if (validateForm()) {
            ...
            try {
                ...
                enqueueSnackbar('Avaliação cadastrada com sucesso!', { variant: 'success' });
                closeDrawer();
                refreshInternal?.();
            } catch (error: unknown) {
                setAttemptCount(prevCount => prevCount + 1);
                if (attemptCount >= 5) {
                    return enqueueSnackbar('Erro ao cadastrar avaliação. Entre em contato com nosso suporte.', { variant: 'error' });
                }

                const genericError = 'Ops! Algo deu errado ao cadastrar avaliação. Tente novamente!'
                const errorMessage = GetErrorMessage(error, genericError);
                enqueueSnackbar(errorMessage, { variant: 'error' });
            } finally {
                setIsLoading(false);
