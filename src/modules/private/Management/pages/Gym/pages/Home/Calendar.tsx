...
// Definir o horário de funcionamento
const horarioFuncionamento = { inicio: 7, fim: 22 }; // Das 7h às 22h
...
const [selectedReservation, setSelectedReservation] = useState(null);
    ...
    const handleSaveEdit = () => {
        // Aqui você pode fazer a lógica de salvar a edição do agendamento
        // Por exemplo, atualizando a lista de reservas ou fazendo uma requisição à API
        setOpenEditModal(false);
    };

    const handleDelete = () => {
        // Aqui você pode fazer a lógica de excluir o agendamento
        // Atualizando a lista de reservas ou fazendo uma requisição à API
        setOpenEditModal(false);
    };
    
    

    const formattedDate = (day) => `${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    ...
    const renderReservationStyle = (date) => {
    ...
    const calculateFillPercentage = (dayReservations) => {
        let totalMinutesReserved = 0;

        dayReservations.forEach((reserva) => {
            const start = new Date(reserva.start);
            const end = new Date(reserva.end);

            // Somente considerar horários dentro do horário de funcionamento
            const startHour = Math.max(start.getHours(), horarioFuncionamento.inicio);
            const endHour = Math.min(end.getHours(), horarioFuncionamento.fim);

            if (startHour < endHour) {
            totalMinutesReserved += (endHour - startHour) * 60;
            }
        });

        // Total de minutos no horário de funcionamento
        const totalMinutesAvailable = (horarioFuncionamento.fim - horarioFuncionamento.inicio) * 60;

        // Calcular a porcentagem preenchida
        return (totalMinutesReserved / totalMinutesAvailable) * 100;
    };
