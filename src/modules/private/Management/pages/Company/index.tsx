...
import { useCompanyGymManagement } from "../Gym/hooks";
                    ...
                    <Box className='w-full md:mt-0 mt-5'>
                        {isCompanyLoading ? (
                                ...
                                <>
                                    ...
                                    {data?.findCompanyDetails.ownershipType === 'MAIN'
                                        ? data?.findCompanyDetails.contact[0]?.email
                                        ? `Matriz | ${data.findCompanyDetails.contact[0].email}`
                                        : 'Matriz'
                                        : data?.findCompanyDetails.contact[0]?.email
                                        ? `Filial | ${data.findCompanyDetails.contact[0].email}`
                                        : 'Filial'}
                                    </Typography>
                                </>
                            )
                ...
                <Box className='flex flex-row w-full mt-5'>
                    <Box className='bg-white w-full rounded-3xl shadow-md p-5 border border-[#EAECF0]'>
                        ...
                        <Box className='mt-5 border border-neutral-300 rounded-lg p-5'>
                            <Box className='!text-neutral20 !font-roboto !text-base !font-semibold !flex !items-center !gap-4'>Dados da Empresa
                                <Button
                            ...
                            {isCompanyLoading ? (
                                    ...
                                    <>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>CNPJ</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>
                                                {data?.findCompanyDetails.code ? FormatCode(data?.findCompanyDetails.code) : ''}
                                                </Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Nome Fantasia</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.fantasyName || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Início da Vigência</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.startDate || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Fim da Vigência</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.endDate || ''}</Typography>
                                        </Box>
                                    </>
                                )
                            }
                            ...
                            {isCompanyLoading ? (
                                    ...
                                    <>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            ...
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>
                                                {data?.findCompanyDetails.zipCode ? FormatZipCode(data.findCompanyDetails.zipCode) : ''}
                                            </Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Logradouro</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.address || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Número</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.number || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Complemento</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.complement || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Bairro</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.district || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Cidade</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.city || ''}</Typography>
                                        </Box>
                                        <Box className='grid grid-cols-[10rem,1fr]'>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4'>Estado</Typography>
                                            <Typography className='!text-neutral-700 !font-roboto !text-sm !mt-4 !font-semibold'>{data?.findCompanyDetails.state || ''}</Typography>
                                        </Box>
                                    </>
                                )
                            }
                        ...
                        <Box className='mt-5 border border-neutral-300 rounded-lg p-5 pb-0'>
                            <Box className='!text-neutral20 !font-roboto text-base font-semibold flex md:items-center !gap-4 mb-3 md:flex-row flex-col justify-between'>
                                ...
                                {data && data.findCompanyDetails?.contact?.filter((contact) => contact.emailStatus === 'PENDING').length > 0 && (
                                    <Box className='bg-[#fff9ee] border border-[#faa200] rounded-lg font-semibold flex flex-row items-center justify-center text-[#faa200] py-1 px-2 uppercase text-[.8rem] font-poppins'>
                                        ...
                                        {data.findCompanyDetails?.contact?.filter((contact) => contact.emailStatus === 'PENDING').length > 1 
                            ...
                            {isCompanyLoading ? (
                                    ...
                                    <Box className='flex flex-wrap justify-between'>
                                        {data?.findCompanyDetails.contact
