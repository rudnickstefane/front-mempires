...
import { useFourStepConfigGym } from '../hooks';


export const FourStepConfigGym = ({ formData, setFormData, errors, setErrors, handleContinue, handleBack }: StepFourProps) => {

    const {
        modalities,
        handleCheckboxChange,
        validateForm,
    } = useFourStepConfigGym(formData, setFormData, setErrors);
    ...
    return (
        <Box className='flex flex-col w-[80vw]'>
            <Box className='w-full rounded-lg shadow-lg bg-white p-6 md:p-10'>
                            ...
                            <Box key={index} className='mb-3'>
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            checked={
                                                formData.selectedModalities?.some(
                                                    (selected) => selected.name === modality.name
                                                ) || false
                                            }
                ...
                {errors.modalitiesError && <p className='text-[#d32f2f] text-[0.75rem]'>{errors.modalitiesError}</p>}
