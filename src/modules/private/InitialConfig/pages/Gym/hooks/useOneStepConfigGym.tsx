...
import { formatTime } from "@sr/utils/format-time.util";
...
export const useOneStepConfigGym = (
  formData: StepOneProps["formData"],
  setFormData: StepOneProps["setFormData"],
  setErrors: StepOneProps["setErrors"]
) => {
  const [selectedValue, setSelectedValue] = useState(
    formData.isPersonalizable ? "isPersonalizable" : "is24Hours"
  );
  ...
  const handleTextFieldChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prevState) => ({
      ...prevState,
      [name]:
        name.startsWith("startTime") || name.startsWith("endTime")
          ? formatTime(value)
          : value,
    }));
  ...
  const handleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prevState) => ({
      ...
      startTimeMondayToFriday:
        value === "is24Hours" ? "00:00" : prevState.startTimeMondayToFriday,
      endTimeMondayToFriday:
        value === "is24Hours" ? "23:59" : prevState.endTimeMondayToFriday,
      startTimeSaturday:
        value === "is24Hours" ? "00:00" : prevState.startTimeSaturday,
      endTimeSaturday:
        value === "is24Hours" ? "23:59" : prevState.endTimeSaturday,
      startTimeSunday:
        value === "is24Hours" ? "00:00" : prevState.startTimeSunday,
  ...
  const validateForm = () => {
    const newErrors: GymConfigErrorStepOne = {
      startTimeMondayToFridayError: formData.startTimeMondayToFriday
        ? null
        : validateTime(formData.startTimeMondayToFriday),
