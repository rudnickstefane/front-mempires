...
import {
  GymConfigErrorStepOne,
  GymConfigErrorStepTwo,
} from "../../../../../../pages/sign-up/gym/types";
import {
  GymConfigErrorStepFour,
  GymConfigErrorStepThree,
} from "../../../../../../pages/sign-up/gym/types/gym-errors.types";
import {
  GymStepOneFormsConfig,
  GymStepOthersFormsConfig,
  GymStepThreeFormsConfig,
  GymStepTwoFormsConfig,
} from "../../../../../../pages/sign-up/gym/types/gym-forms-config.types";
import {
...
export const useSignUpConfigGym = () => {
  ...
  const [stepThreeErrors, setStepThreeErrors] =
    useState<GymConfigErrorStepThree>({
      workFormError: "",
      plansError: "",
    });
  ...
  const [stepThreeFormData, setStepThreeFormData] =
    useState<GymStepThreeFormsConfig>({
  ...
  const validateForm = () => {
    ...
    return !(newErrors.servicesError, newErrors.segmentsError);
  };
  ...
  const handleFinish = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        const response: InitialConfigureResponse = await request(
          MutationInitialConfigure,
          variables
        );
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao configurar sua academia. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao configurar sua academia. Tente novamente!";
