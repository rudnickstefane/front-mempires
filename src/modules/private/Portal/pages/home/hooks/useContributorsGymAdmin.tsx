...
export const useContributorsGymAdmin = ({
  ...
  const [responsePlans, setResponseContributors] =
    useState<FindContributorsResponse | null>();
  const [alterContributor, setalterContributor] =
  ...
  const filteredPlans = (responsePlans?.findContributors || []).filter(
    (student) => {
      ...
      return (
        ...
        (student.profileCode &&
          ...
          student.profileCode.toString() === extractedProfileCode) ||
  ...
  const handleMoreDetails = (profileCode: string) => {
    const studentDetail = responsePlans?.findContributors?.find(
      (student) => student.profileCode === profileCode
    );
    if (studentDetail) {
  ...
  const handleAlterContributor = (profileCode: string) => {
    const studentDetail = responsePlans?.findContributors?.find(
      (plan) => plan.profileCode === profileCode
    );
    if (studentDetail) {
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar colaborador. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar colaborador. Tente novamente!";
  ...
  const handleDeletePlan = async (profileCode: string) => {
    try {
      ...
      enqueueSnackbar("Colaborador excluído com sucesso", {
        variant: "success",
      });
      refresh("findContributors");
    } catch (error: unknown) {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir colaborador. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir colaborador. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const findContributors = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindContributorsResponse = await request(
        QueryFindContributors,
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar colaboradores. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar(
        "Erro ao buscar colaboradores cadastrados em sua academia.",
        { variant: "error" }
      );
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    if (responsePlans) {
      const studentDetail = responsePlans.findContributors?.find(
        (student) => student.profileCode === contributorDetails?.profileCode
      );
