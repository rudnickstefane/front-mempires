...
export const useGroupsGymAdmin = ({
  ...
  const [responseGroups, setResponseGroups] =
    useState<FindGroupsResponse | null>();
  ...
  const filteredPlans = (responseGroups?.findGroups || []).filter((group) => {
    ...
    return (
      ...
      (group.groupCode &&
        extractedGroupCode &&
        group.groupCode.toString() === extractedGroupCode)
    );
  });
  ...
  const handleMoreDetails = (groupCode: string) => {
    const groupDetail = responseGroups?.findGroups?.find(
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar grupo. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar grupo. Tente novamente!";
  ...
  useEffect(() => {
    if (responseGroups) {
      const groupDetail = responseGroups.findGroups?.find(
        (group) => group.groupCode === groupDetails?.groupCode
      );
