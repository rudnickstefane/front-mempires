...
import { EditCompanyDrawer } from "../../../components/Drawer";
...
export const useCompanyGymManagement = ({
  data,
  refresh,
  ...
  const handleCrop = async () => {
    if (cropperRef.current) {
      ...
      try {
        ...
        enqueueSnackbar("Foto de perfil alterada com sucesso!", {
          variant: "success",
        });
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao alterar foto de perfil. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }
        enqueueSnackbar(
          "Ops! Algo deu errado ao alterar sua foto de perfil. Tente novamente!",
          { variant: "error" }
        );
      } finally {
        setIsLoading(false);
  ...
  const handleDeleteImage = async () => {
    try {
      ...
      enqueueSnackbar("Foto de perfil excluída com sucesso!", {
        variant: "success",
      });
      refresh?.();
    } catch (error: unknown) {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir de perfil. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir de perfil. Tente novamente!";
