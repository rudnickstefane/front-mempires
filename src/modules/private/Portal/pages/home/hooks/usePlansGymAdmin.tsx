...
export const usePlansGymAdmin = ({
  ...
  const [responsePlans, setResponsePlans] =
    useState<FindPlansResponse | null>();
  ...
  const filteredPlans = (responsePlans?.findPlans || []).filter((plan) => {
    ...
    return (
      ...
      (plan.planCode &&
        extractedPlanCode &&
        plan.planCode.toString() === extractedPlanCode) || // Filtrar por PLN-52
      (plan.periodicities &&
        plan.periodicities.some(
          (periodicity) => periodicity.amount?.toString().includes(searchText) // Filtrar por periodicidade
        ))
    );
  });
  ...
  const handleMoreDetails = (planCode: string) => {
    const planToAlter = responsePlans?.findPlans?.find(
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar plano. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar plano. Tente novamente!";
  ...
  const handleDeletePlan = async (planCode: string) => {
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir plano. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir plano. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  useEffect(() => {
    if (responsePlans) {
      const planToAlter = responsePlans.findPlans?.find(
        (plan) => plan.planCode === planDetails?.findPlans?.planCode
      );
