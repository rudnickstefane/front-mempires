...
export const useClassesGymAdmin = ({
  ...
  const [responseClasses, setResponseClasses] =
    useState<FindClassesResponse | null>();
  ...
  const filteredClasses = (responseClasses?.findClasses || []).filter(
    (classes) => {
      ...
      return (
        ...
        (classes.classCode &&
          extractedclassCode &&
          classes.classCode.toString() === extractedclassCode)
      );
    }
  );
  ...
  const handleMoreDetails = (classCode: string) => {
    const classToAlter = responseClasses?.findClasses?.find(
      (classes) => classes.classCode === classCode
    );
    if (classToAlter) {
      setclassDetails({
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar turma. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar turma. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };

  const handleStatusDay = async (
    classCode: string,
    day: string,
    status: boolean
  ) => {
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar o dia. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar o dia. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const handleDeletePlan = async (classCode: string) => {
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir turma. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir turma. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const findClasses = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindClassesResponse = await request(QueryFindClasses, {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar turmas. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar turmas cadastradas em sua academia.", {
        variant: "error",
      });
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    if (responseClasses) {
      const classToAlter = responseClasses.findClasses?.find(
        (classes) => classes.classCode === classDetails?.findClasses?.classCode
      );
    ...
    classDetails?.findClasses,
    classDetails?.findClasses?.classCode,
    responseClasses,
  ]);
