...
export const useVisitsGymAdmin = ({
  ...
  const [responsePlans, setResponsePlans] =
    useState<FindVisitsResponse | null>();
  ...
  const filteredPlans = (responsePlans?.findVisits || []).filter((visit) => {
    ...
    return (
      ...
      (visit.visitCode &&
        ...
        visit.visitCode.toString() === extractedProfileCode) ||
  ...
  const handleMoreDetails = (visitCode: string) => {
    const visitDetails = responsePlans?.findVisits?.find(
  ...
  const handleAlterVisit = (visitCode: string) => {
    const studentDetail = responsePlans?.findVisits?.find(
      (visit) => visit.visitCode === visitCode
    );
    if (studentDetail) {
  ...
  const findVisits = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindVisitsResponse = await request(QueryFindVisits, {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar visitantes. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar visitantes em sua academia.", {
        variant: "error",
      });
    } finally {
      setIsLoading(false);
