...
export const useCoreTicket = ({
  ...
  const [responseTickets, setResponseTickets] =
    useState<FindTicketsResponse | null>();
  const [responseSuggestions, setResponseSuggestions] =
    useState<FindTicketsResponse | null>();
  const [responseUpdates, setResponseUpdates] =
    useState<FindUpdatesResponse | null>();
  ...
  const filteredPlans = (responseTickets?.findTickets || []).filter(
    (ticket) => {
      ...
      return (
        ...
        (ticket.ticketCode &&
          extractedTicketCode &&
          ticket.ticketCode.toString() === extractedTicketCode)
      );
    }
  );
  ...
  const handleMoreDetails = (ticketCode: string, type: string) => {
    if (type === "PROTOCOL") {
      const ticketDetail = responseTickets?.findTickets?.find(
    ...
    if (type === "SUGGESTION") {
      const suggestionDetail = responseSuggestions?.findTickets?.find(
        (ticket) => ticket.ticketCode === ticketCode
      );
      if (suggestionDetail) {
    ...
    if (type === "UPDATES") {
      const updateDetail = responseUpdates?.findUpdates?.find(
        (ticket) => ticket.updateCode === ticketCode
      );
      if (updateDetail) {
  ...
  const findTickets = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindTicketsResponse = await request(QueryFindTickets, {
        request: "COMPANY_RECIPIENT_TO_ALL",
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar protocolos. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
  ...
  const findUpdates = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindUpdatesResponse = await request(QueryFindUpdates, {
        request: role,
      });
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao encontrar atualizações. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
  ...
  const findSuggestions = useCallback(async () => {
    ...
    try {
      const response: FindTicketsResponse = await request(QueryFindTickets, {
        request: "COMPANY_ISSUER_SUGGESTION_TO_ALL",
        issuerCode: companyCode,
      });
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar sugestões. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
  ...
  useEffect(() => {
    if (responseTickets) {
      const ticketMessageReceived = responseTickets.findTickets?.find(
        (ticket) => ticket.ticketCode === ticketDetails?.ticketCode
      );
    ...
    if (responseSuggestions) {
      const ticketMessageReceived = responseSuggestions.findTickets?.find(
        (ticket) => ticket.ticketCode === suggestionDetails?.ticketCode
      );
