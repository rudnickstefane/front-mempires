...
export const useStudentsGymAdmin = ({
  ...
  const [responsePlans, setResponsePlans] =
    useState<FindStudentsResponse | null>();
  ...
  const filteredPlans = (responsePlans?.findStudents || []).filter(
    (student) => {
  ...
  const handleMoreDetails = (profileCode: string) => {
    const studentDetail = responsePlans?.findStudents?.find(
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar aluno. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar aluno. Tente novamente!";
  ...
  const handleDeletePlan = async (profileCode: string) => {
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir aluno. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir aluno. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const findStudents = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindStudentsResponse = await request(QueryFindStudents, {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar alunos. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar alunos matriculados em sua academia.", {
        variant: "error",
      });
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    if (responsePlans) {
      const studentDetail = responsePlans.findStudents?.find(
        (student) => student.profileCode === studentDetails?.profileCode
      );
