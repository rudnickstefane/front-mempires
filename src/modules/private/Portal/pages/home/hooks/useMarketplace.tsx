...
export const useMarketplace = ({
  ...
  const [responseProducts, setResponseProducts] =
    useState<FindProductsResponse | null>();
  ...
  const filteredProducts = (responseProducts?.findProducts || []).filter(
    (product) => {
      ...
      return (
        ...
        (product.productCode &&
          ...
          product.productCode.toString() === extractedproductCode)
      );
    }
  );
  ...
  const handleMoreDetails = (productCode: string) => {
    const studentDetail = responseProducts?.findProducts?.find(
  ...
  const findProducts = useCallback(async () => {
    setIsLoading(true);
    try {
      const response: FindProductsResponse = await request(QueryFindProducts, {
        request: "MARKETPLACE",
      });
