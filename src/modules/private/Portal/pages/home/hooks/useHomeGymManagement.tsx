...
import {
  FindSlidesResponse,
  FindStudentMetricsResponse,
  FindUserShortcutsResponse,
  StudentMetrics,
} from "../../../../../common/types";
...
import {
  MutationDeleteShortcut,
  QueryFindSlides,
  QueryFindStudentsMetrics,
  QueryFindUserShortcuts,
...
export const useHomeGymManagement = ({
  ...
  const { request } = useBackend(jwtSecret);
  ...
  const [responseStudentMetrics, setResponseStudentMetrics] =
  ...
  const [responseShortcuts, setResponseShortcuts] =
    useState<FindUserShortcutsResponse>();
  ...
  const findMetrics = useCallback(async () => {
    ...
    try {
      const response: FindStudentMetricsResponse = await request(
        QueryFindStudentsMetrics,
      ...
      enqueueSnackbar("Ocorreu um problema ao exibir os gráficos dos alunos.", {
        variant: "error",
      });
    }
  ...
  const findUserShortcuts = useCallback(async () => {
    ...
    try {
      const response: FindUserShortcutsResponse = await request(
        QueryFindUserShortcuts,
      ...
      enqueueSnackbar("Não foi possível exibir seus atalhos.", {
        variant: "error",
      });
    }
  ...
  const findSlides = useCallback(async () => {
    ...
    try {
      const response: FindSlidesResponse = await request(QueryFindSlides, {
        origin: "GYM",
      });
      ...
      enqueueSnackbar("Tivemos um problema ao exibir os slides.", {
        variant: "error",
      });
    }
  ...
  const handleDeleteShortcut = async (shortcut: { shortcutCode: string }) => {
    try {
      ...
      await request(MutationDeleteShortcut, {
        shortcutCode,
        profileCode: profileCode,
      });
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir atalho. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir atalho. Tente novamente!";
