...
import { useMarketplace } from "../home/hooks";
import { GymManagementProps } from "../home/types";

export default function Marketplace({ enqueueSnackbar }: GymManagementProps) {
  const { searchText, setSearchText, productsToDisplay } = useMarketplace({
    enqueueSnackbar,
  });
  ...
  const getQuantityByType = (type: string) => {
    return (
      productsToDisplay?.filter((product) => product.type === type).length || 0
    );
  };
      ...
      <Box className="overflow-x-auto max-h-[calc(100vh-60px)] p-5 pb-[4rem]">
        ...
        <Box className="flex flex-row w-full mt-5">
          <Box className="mr-5 w-[284px] flex flex-col items-start">
            ...
            <Typography className="!mb-3 !font-semibold !mt-10">
              Custo do frete
            </Typography>
            <Typography className="!mb-3 !font-semibold !mt-10">
              Localização
                    ...
                    <Box className="mt-5 flex-col">
                      ...
                      {product.freeInstallments === product.installments ? (
                        <Typography className="flex flex-row !text-[.84rem] text-neutral-600">
                          em{" "}
                          <Box className="text-black ml-1">
                            {product.freeInstallments}x de{" "}
                            {Number(
                              Number(product.price) /
                                Number(product.installments) || 0
                            ).toLocaleString("pt-BR", {
                              style: "currency",
                              currency: "BRL",
                            })}
                        ...
                        <Typography className="flex flex-row !text-[.84rem] text-neutral-600">
                          em{" "}
                          <Box className="text-[#00a650] ml-1">
                            {product.freeInstallments}x sem juros de{" "}
                            {Number(
                              Number(product.price) /
                                Number(product.installments) || 0
                            ).toLocaleString("pt-BR", {
                      ...
                      <Typography className="flex flex-row !text-[.84rem] !mt-4 text-neutral-500">
                        Enviado por:{" "}
                        {product.shippingType || "Logística própria"}
                      </Typography>
                    </Box>
                  </Box>
                </Button>
              ...
              <Box className="w-full flex justify-center mt-20">
                <Box className="flex flex-col items-center">
                  ...
                  <Box className="flex flex-col mt-5">
                    <Typography>
                      No momento, não temos produtos disponíveis. Tente
                      novamente mais tarde ou explore outras opções.
                    </Typography>
                  </Box>
                </Box>
              </Box>
