...
import { HomeGymAdminType } from "../home/pages/Admin/types/AdminGym.types";
import { ResourceBoxProps } from "../home/types/gym-resource-box.types";
...
const renderComponents: {
  [key in "Protocols" | "Updates" | "Suggestions"]: React.ComponentType;
} = {
  Protocols: Protocols,
  Updates: Updates,
...
export default function Support() {
  const [selectedType, setSelectedType] =
    useState<HomeGymAdminType>("Protocols"); // Default to 'Visits'

  const relationshipResources = subResources.filter(
    (resource) => resource.menu === "Relationship"
  );
  const SelectedComponent =
    renderComponents[selectedType as keyof typeof renderComponents];
              ...
              <Box className="flex flex-col min-w-[13.5rem] h-full mr-0">
                ...
                {relationshipResources.map((resource) => (
                  <SubResourceBox
                    ...
                    {...resource}
                    onClick={() =>
                      setSelectedType(resource.type as HomeGymAdminType)
                    }
