...
export const usePlanCreateForm = ({
  ...
  const [selectedChargeValue, setSelectedChargeValue] = useState(
    formData.isPlan ? "isPlan" : "isPeriodicity"
  );
  ...
  const [selectedFeesFrequencyValue, setSelectedFeesFrequencyValue] = useState(
    formData.isUnique ? "isUnique" : "isDaily"
  );
  const [selectedCalculationBaseValue, setSelectedCalculationBaseValue] =
    useState(formData.isValue ? "isValue" : "isPercentage");
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormPlanCreate(
      formData,
      activeStep,
      startTime,
      endTime,
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Cadastro do plano realizado com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao cadastrar plano. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao cadastrar o plano. Tente novamente!";
