...
export const useStudentRegisterForm = ({
  ...
  const [responseFinanceResponsible, setResponseFinanceResponsible] =
  ...
  const [responseStudent, setResponseStudent] =
    useState<FindStudentResponse | null>();
  const companyCode = Number(localStorage.getItem("@iflexfit:companyCode"));
  const [responsePlansOptions, setResponsePlansOptions] = useState<
  ...
  const referralSourceOptions = [
    {
      value: "noSelect",
      label: "Selecione como nos conheceu",
      isDisabled: true,
    },
  ...
  const handleTextFieldChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "zipCode" && updatedValue.length === 9) {
      axios
        .get(`${APIS.zipCode}/${updatedValue}/json/`)
        .then((response) => {
    ...
    if (name === "identity" && value) {
      ...
      if (!containsLetters) {
        ...
        if (startsWithTwoLetters && trimmedValue.length === 8) {
          const { formatted: passFormatted, isValid: passIsValid } =
            ValidatePassport(trimmedValue);
    ...
    if (name === "financeResponsible" && value) {
      ...
      if (Number(identity.length) === 11 && isValid) {
        setIsLoading(true);
        try {
          const response: FindStudentResponse = await request(
            QueryFindStudent,
            { companyCode: companyCode, search: identity }
          );
          ...
          if (attemptCount >= 5) {
            return enqueueSnackbar(
              "Erro ao localizar responsável financeiro. Entre em contato com nosso suporte.",
              { variant: "error" }
            );
          }
          ...
          const genericError =
            "Ops! Algo deu errado ao localizar o responsável financeiro. Tente novamente!";
  ...
  const handleSearchChange = useDebounce(
    async (value: string, isValid: boolean) => {
      ...
      try {
        ...
        const genericError =
          "Ops! Algo deu errado ao localizar aluno. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      }
    },
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormRegisterStudent(
      formData,
      activeStep,
      indicationCode,
      isIndication
  ...
  const findPlans = useCallback(async () => {
    try {
      const response: FindPlansResponse = await request(QueryFindPlans, {
        companyCode: companyCode,
      });
        ...
        .flatMap((plan) =>
          plan.periodicities.map((periodicity) => {
            ...
            const periodicityName = periodicity.name.includes("meses")
              ? `Pagamento a cada ${periodicity.name
                  .replace("meses", "")
                  .trim()} meses`
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar planos. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar planos cadastrados em sua academia.", {
        variant: "error",
      });
    }
  ...
  const [selectedPaymentValue, setSelectedPaymentDayValue] = useState(
    formData.paymentDay ? "programmed" : "today"
  );
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      setIsLoading(true);
      try {
        const financeResponsibleCode =
          responseFinanceResponsible?.findStudent.profileCode;
        const indicationCode = responseStudent?.findStudent.profileCode;
        const variables = StudentUpsertVariables(
          formData,
          financeResponsibleCode,
          indicationCode
        );
        await request(MutationStudentUpsert, variables);
        enqueueSnackbar("Cadastro do aluno realizado com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao matricular o aluno. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao matricular o aluno. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      } finally {
        setIsLoading(false);
