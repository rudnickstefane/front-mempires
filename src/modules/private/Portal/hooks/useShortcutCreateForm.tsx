...
export const useShortcutCreateForm = ({
  ...
  const [responseShortcuts, setResponseShortcuts] =
    useState<FindShortcutsResponse>();
  ...
  const findShortcuts = useCallback(async () => {
    try {
      const response: FindShortcutsResponse = await request(
        QueryFindShortcuts,
      ...
      try {
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao salvar novo atalho. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao salvar novo atalho. Tente novamente!";
