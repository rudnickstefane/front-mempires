...
import {
  ...
  FormatText,
  GetErrorMessage,
  ValidateFormReversedTransaction,
} from "../../../common/utils";
  ...
  const handleTextFieldChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "reversed") {
      ...
      const mainAmountValue =
        parseFloat(data.mainAmount.replace(/\./g, "").replace(",", ".")) || 0;

      const secondaryAmountValue =
        parseFloat(data.secondaryAmount.replace(/\./g, "").replace(",", ".")) ||
        0;

      const maxAmount = mainAmountValue + secondaryAmountValue;

      if (reversedValue > maxAmount) {
        updatedValue = FormatCash(
          maxAmount.toFixed(2).toString().replace(".", ",")
        );
      } else {
        const maxLength = (data.mainAmount + data.secondaryAmount).replace(
          /\D/g,
          ""
        ).length;
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        const variables = {
          data: {
            ...
            input: {
              ...
              reversed: parseFloat(
                formData.reversed?.replace(/[R$.\s]/g, "").replace(",", ".") ||
                  "0"
              ),
        ...
        enqueueSnackbar("Transação reembolsada com sucesso", {
          variant: "success",
        });
        refreshInternal?.();
        closeDrawer();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao reembolsar transação. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao reembolsar transação. Tente novamente!";
