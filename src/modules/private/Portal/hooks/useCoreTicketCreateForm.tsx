...
import { MutationTicketUpsert } from "../pages/home/graphql/MutationTicketUpsert";
import { TicketUpsertVariables } from "../pages/home/utils";

export const useCoreTicketCreateForm = ({
  closeDrawer,
  enqueueSnackbar,
  data,
  refresh,
  ...
  const [formData, setFormData] = useState<TicketsProps["formData"]>({
    ...
    recipientCompanyCode: data?.recipientCompanyCode
      ? data.recipientCompanyCode
      : null,
    name: "",
    description: "",
    duration: "",
    ...
    receivedUserCode: data?.issuerUserCode ? data.issuerUserCode : null,
    receivedCompanyCode: data?.issuerCompanyCode
      ? data.issuerCompanyCode
      : null,
  });
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Protocolo aberto com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao abrir protocolo. Entre em contato por outro método de comunicação.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao abrir protocolo. Tente novamente!";
  ...
  const handleSuggestionCreate = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        const variables = TicketUpsertVariables(ticket);
        await request(MutationTicketUpsert, variables);
        enqueueSnackbar("Sugestão enviada com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao enviar sugestão. Entre em contato por outro método de comunicação.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao enviar sugestão. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      } finally {
        setIsLoading(false);
  ...
  const handleMessageCreate = async (ticketCode: string) => {
    ...
    try {
      const ticket = {
        action: "CORE_TICKET_MESSAGE_CREATE",
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao enviar mensagem. Entre em contato por outro método de comunicação.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao enviar mensagem. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    } finally {
      setIsLoading(false);
  ...
  const notificationPush = async () => {
    const variables = {
      input: {
        ...
        issuerUserCode: profileCode,
        issuerCompanyCode: companyCode,
        recipientUserCode: data?.issuerUserCode ? data.issuerUserCode : null,
        recipientCompanyCode: data?.issuerCompanyCode
          ? data.issuerCompanyCode
          : null,
        path: "Support",
      },
