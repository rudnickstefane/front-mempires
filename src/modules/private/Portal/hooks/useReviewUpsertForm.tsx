...
export const useReviewUpsertForm = ({
  closeDrawer,
  enqueueSnackbar,
  data,
  refreshInternal,
  ...
  const dateToday = new Date(); // 11/03/2025
  const monthsDueDate = new Date(dateToday.setMonth(dateToday.getMonth() + 3)); // Adiciona 3 meses

  const dueDate = monthsDueDate.toISOString().split("T")[0];
  ...
  const handleTextFieldChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!hasBirthdayPassed) {
      age--; // Caso ainda não fez aniversário este ano
    }
    ...
    if (formData.protocol === "P954") {
      ...
      const ajustePorTMB = (taxametabólicabasal - 1200) / 100;
      const ajustePorGordura = (massagordaKg - 20) / 2;
      const idadecorporal = Math.max(10, age + ajustePorTMB + ajustePorGordura);
    ...
    if (formData.protocol === "L964") {
      ...
      const density = 1.1862 - 0.0684 * Math.log10(sd) - 0.000601 * age;
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      setIsLoading(true);
      try {
        const variables = ReviewCreateVariables(formData, "CREATE");
        await request(MutationReviewUpsert, variables);

        enqueueSnackbar("Avaliação cadastrada com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refreshInternal?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao cadastrar avaliação. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao cadastrar avaliação. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      } finally {
        setIsLoading(false);
