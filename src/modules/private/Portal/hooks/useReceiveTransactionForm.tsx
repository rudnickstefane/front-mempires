...
export const useReceiveTransactionForm = ({
  ...
  const dynamicSteps = [
    formData.receiveType ? formData.receiveType : "Opções",
    "Recebimento",
  ];

  const handleProofPaymentUpload = (
  ...
  const [selectedRewardsValue, setSelectedRewardsValue] = useState(
    formData.yesConvert ? "yesConvert" : "noConvert"
  );
  ...
  const handleTextFieldChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const newUpdatedValue =
      parseFloat(updatedValue.replace(/\./g, "").replace(",", ".")) || 0;

    const totalTransaction =
      parseFloat(data.amount.replace(/\./g, "").replace(",", ".")) || 0;

    const totalRewards =
      parseFloat(data.rewardsCredit.replace(/\./g, "").replace(",", ".")) || 0;

    let mainAmount = formData.mainAmount
      ? parseFloat(formData.mainAmount.replace(/\./g, "").replace(",", "."))
      : 0;
    let secondaryAmount = formData.secondaryAmount
      ? parseFloat(
          formData.secondaryAmount.replace(/\./g, "").replace(",", ".")
        )
      : 0;
    const fees = formData.fees
      ? parseFloat(formData.fees.replace(/\./g, "").replace(",", "."))
      : 0;
    ...
    if (
      combinePaymentMethod &&
      formData.secondaryPaymentMethod === "REWARDS_CREDIT"
    ) {
      if (name === "secondaryAmount") {
        secondaryAmount = newUpdatedValue;
  ...
  const handleSelectChange = (event: SelectChangeEvent<string>) => {
    ...
    setFormData((prevState) => {
      ...
      if (
        name === "paymentMethod" &&
        prevState.secondaryPaymentMethod !== "CARD"
      ) {
        return {
          ...prevState,
          [name]: value,
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormReceiveTransaction(
      formData,
      activeStep,
      combinePaymentMethod
    );
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      const changeOrRewards = parseFloat(
        formData.change?.replace(/[R$.\s]/g, "").replace(",", ".") || "0"
      );
      const rewardsCredit =
        selectedRewardsValue === "yesConvert" ? changeOrRewards : 0;
      ...
      try {
        const variables = {
          data: {
            ...
            input: {
              ...
              action: "RECEIVE",
              mainAmount: parseFloat(
                formData.mainAmount
                  ?.replace(/[R$.\s]/g, "")
                  .replace(",", ".") || "0"
              ),
              secondaryAmount: parseFloat(
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao processar pagamento transação. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao processar pagamento da transação. Tente novamente!";
