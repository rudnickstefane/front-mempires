import { formatIdentity } from "@sr/utils";
import { VariantType } from "notistack";
import { ChangeEvent, useState } from "react";
import {
  FormatAndValidateCodeAndIdentity,
  ValidateFormSupplierRegister,
} from "../../../common/utils";
...
const mockData = {
  ...
  address:
    "Av. Barão Homem de Melo, nº 1985 - Alpes, Belo Horizonte - MG, 30451-669",
};
...
export const useSupplierRegisterForm = ({
  ...
  const [supplierData, setSupplierData] = useState<null | typeof mockData>(
    null
  );
  ...
  const handleTextFieldChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "codeAndIdentity" && value) {
      ...
      if (
        ...
        (formatted.length === 11 && isValid) ||
        (value.length === 14 && isValid) ||
        (formatted.length === 18 && isValid)
      ) {
        setIsLoading(true);
        ...
        setTimeout(() => {
          ...
          if (value === mockData.cpf || value === mockData.cnpj) {
            const formattedCNPJ = mockData.cnpj
              ? FormatCode(mockData.cnpj)
              : "";
            const formattedCPF = mockData.cpf
              ? formatIdentity(mockData.cpf)
              : "";
            setSupplierData({
              ...mockData,
              cnpj: formattedCNPJ,
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Fornecedor vinculado com sucesso!", {
          variant: "success",
        });
        closeDrawer();
      } catch (error) {
