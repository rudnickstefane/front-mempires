...
import {
  ...
  ValidateFormReviewUpsert,
} from "../../../common/utils";
import { ReviewsProps, StudentRegisterProps } from "../components/Drawer/types";
import {
  MutationReviewUpsert,
  QueryFindReviewQuestions,
} from "../components/Graphql";
import { QueryFindStudent } from "../pages/home/graphql";
import { FindStudentResponse } from "../pages/home/types";
...
export const useReviewEditForm = ({
  ...
  const [responseEvaluator, setResponseEvaluator] =
    useState<FindStudentResponse | null>();
  const [activeStep, setActiveStep] = useState(0);
  const [attemptCount, setAttemptCount] = useState(0);
  const [responseReviewQuestions, setResponseReviewQuestions] =
    useState<FindReviewQuestionsResponse | null>();
  ...
  const formatMeasurement = (question: string) => {
    const value = data.response.find(
      (item: ResponseItem) =>
  ...
  const [formData, setFormData] = useState<ReviewsProps["formData"]>({
    ...
    mo: "",
    vo: data.vo,
    abdominalrepetições: data.response.find(
      (item: ResponseItem) =>
        item.category === "NEUROMOTORS" &&
        item.question === "Abdominal Repetições"
    )?.response,
    flexãodebraçosrepetições: data.response.find(
      (item: ResponseItem) =>
        item.category === "NEUROMOTORS" &&
        item.question === "Flexão de Braços Repetições"
    )?.response,
    sentarealcançar: data.response.find(
      (item: ResponseItem) =>
        item.category === "NEUROMOTORS" && item.question === "Sentar e Alcançar"
    )?.response,
    ...
    massamagrakg: Number(
      (data.leanMass / 100) * data.weight.replace(",", ".")
    ).toFixed(2),
    massagordakg: Number(
      (data.fatMass / 100) * data.weight.replace(",", ".")
    ).toFixed(2),
    ...
    subscapularis: data.subscapularis
      ? Number(data.subscapularis).toFixed(1).replace(".", ",")
      : "",
    triceps: data.triceps
      ? Number(data.triceps).toFixed(1).replace(".", ",")
      : "",
    chest: data.chest ? Number(data.chest).toFixed(1).replace(".", ",") : "",
    middleAxillary: data.middleAxillary
      ? Number(data.middleAxillary).toFixed(1).replace(".", ",")
      : "",
    suprailiac: data.suprailiac
      ? Number(data.suprailiac).toFixed(1).replace(".", ",")
      : "",
    abdominal: data.abdominal
      ? Number(data.abdominal).toFixed(1).replace(".", ",")
      : "",
    medialThigh: data.medialThigh
      ? Number(data.medialThigh).toFixed(1).replace(".", ",")
      : "",
    wristBistyloid: data.wristBistyloid
      ? Number(data.wristBistyloid).toFixed(1).replace(".", ",")
      : "",
    femurBistyloid: data.femurBistyloid
      ? Number(data.femurBistyloid).toFixed(1).replace(".", ",")
      : "",
    medialCalf: data.medialCalf
      ? Number(data.medialCalf).toFixed(1).replace(".", ",")
      : "",
    biceps: data.biceps ? Number(data.biceps).toFixed(1).replace(".", ",") : "",
    abdome: data.abdome ? Number(data.abdome).toFixed(1).replace(".", ",") : "",
    distânciapercorrida: data.response.find(
      (item: ResponseItem) =>
        item.category === "CARDIORESPIRATORY" &&
        item.question === "Distância Percorrida"
    )?.response,
    tempogasto: data.response.find(
      (item: ResponseItem) =>
        item.category === "CARDIORESPIRATORY" && item.question === "Tempo Gasto"
    )?.response,
    frequênciacardíaca: data.response.find(
      (item: ResponseItem) =>
        item.category === "CARDIORESPIRATORY" &&
        item.question === "Frequência Cardíaca"
    )?.response,
    algummédicojádissequevocêpossuialgumproblemadecoraçãoequesódeveriarealizaratividadefísicasupervisionadoporprofissionais:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
            "Algum médico já disse que você possui algum problema de coração e que só deveria realizar atividade física supervisionado por profissionais?"
      )?.response,
    vocêsentedoresnopeitoquandopraticaatividadefísica: data.response.find(
      (item: ResponseItem) =>
        item.category === "PAR_Q" &&
        item.question ===
          "Você sente dores no peito quando pratica atividade física?"
    )?.response,
    noúltimomêsvocêsentiudoresnopeitoquandopraticaatividadefísica:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
    ...
    vocêapresentadesequilíbriodevidoatonturaeouperdadeconsciência:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
    ...
    vocêpossuialgumproblemaósseoouarticularquepoderiaserpioradopelaatividadefísica:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
    ...
    vocêtomaatualmentealgummedicamentoparapressãoarterialeouproblemadecoração:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
    ...
    sabedealgumaoutrarazãopelaqualvocênãodevepraticaratividadefísica:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "PAR_Q" &&
          item.question ===
    ...
    quaissãoseusobjetivoscomrelaçãoaatividadefísica: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Quais são seus objetivos com relação a atividade física?"
    )?.response,
    praticaatividadefísicaatualmentequaiseháquantotempo: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Pratica atividade física atualmente? Quais e há quanto tempo?"
    )?.response,
    realizaacompanhamentocomnutricionistaounutricionistaesportivo:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "ANAMNESE" &&
          item.question ===
            "Realiza acompanhamento com Nutricionista ou Nutricionista Esportivo?"
      )?.response,
    qualseriasuamédiadehorasdesonodiário: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Qual seria sua média de horas de sono diário?"
    )?.response,
    utilizaalgumtipodemedicamentodeusocontinuoqual: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Utiliza algum tipo de medicamento de uso continuo? Qual?"
    )?.response,
    jápassouporalgumtipodecirurgianosúltimos6mesesqual: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Já passou por algum tipo de cirurgia nos últimos 6 meses? Qual?"
    )?.response,
    possuialgumarecomendaçãoourestriçãomédicaparapráticadeexercícios:
      data.response.find(
        (item: ResponseItem) =>
          item.category === "ANAMNESE" &&
          item.question ===
            "Possui alguma recomendação ou restrição médica para prática de exercícios?"
      )?.response,
    possuidiabetesoualgumaoutradoença: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Possui Diabetes ou alguma outra doença?"
    )?.response,
    possuialgumaalteraçãocardíacaqual: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Possui alguma alteração cardíaca? Qual?"
    )?.response,
    existemproblemascardíacosnafamíliaquem: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Existem problemas cardíacos na família? Quem?"
    )?.response,
    suapressãoarterialéaltabaixaounormal12x8: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Sua pressão arterial é alta, baixa ou normal (12x8)?"
    )?.response,
    jásentiuousentedoresnopeito: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Já sentiu ou sente dores no peito?"
    )?.response,
    sentedoresnopeitoaorealizaralgumaatividadefísica: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Sente dores no peito ao realizar alguma atividade física?"
    )?.response,
    sentiuousentedoresnacolunaconstantesemqualregião: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question ===
          "Sentiu ou sente dores na coluna constantes? Em qual região?"
    )?.response,
    jádesmaioualgumavez: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Já desmaiou alguma vez?"
    )?.response,
    éfumanteháquantosanos: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "É fumante? Há quantos anos?"
    )?.response,
    tomaalgumtipodeesteroideanabólico: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Toma algum tipo de esteroide anabólico?"
    )?.response,
    possuialgumtraumaoulesão: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" &&
        item.question === "Possui algum trauma ou lesão?"
    )?.response,
    observações: data.response.find(
      (item: ResponseItem) =>
        item.category === "ANAMNESE" && item.question === "Observações"
    )?.response,
  });
  ...
  const infoCompleted =
    formData.weight != "" && formData.height != "" && formData.profileCode != 0;

  const dynamicSteps = infoCompleted
    ? [
        "Informações",
        "Anamnese",
        "Par-Q",
  ...
  const handleTextFieldChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const isTextField =
      responseReviewQuestions?.findReviewQuestions
        .filter((question) => question.category === "ANAMNESE")
        .some(
          (question) =>
      ...
      name === "tempogasto" ||
      name === "flexãodebraçosrepetições" ||
      name === "abdominalrepetições" ||
    ...
    if (name === "tempogasto") {
      ...
      if (sanitizedValue.length <= 2) {
        ...
        minutes =
          parseInt(sanitizedValue.slice(0, sanitizedValue.length - 2), 10) || 0;
      ...
      if (data.gender === "MAN") {
        ...
        const biestiloidePunho =
          name === "wristBistyloid"
            ...
            : parseFloat(String(formData.wristBistyloid).replace(",", ".")) /
                1000 || 0;
        const biepicondiloFemur =
          name === "femurBistyloid"
            ? parseFloat(updatedValue.replace(",", ".")) / 1000 || 0
            : parseFloat(String(formData.femurBistyloid).replace(",", ".")) /
                1000 || 0;
        ...
        const massaossea =
          3.02 *
          Math.pow(
            height ** 2 * biestiloidePunho * biepicondiloFemur * 400,
            0.712
          );
        const massamuscular =
          weight - (massagorda + massaossea + massaresidual);
        ...
        const idadecorporal = Math.max(
          10,
          data.age + ajustePorTMB + ajustePorGordura
        );
        ...
        const taxametabólicabasal =
          10 * weight + 6.25 * heightCm - 5 * data.age - 161;
    ...
    if (formData.protocol === "JPW78803") {
      const fieldsJPW78803 = ["triceps", "suprailiac", "medialThigh"];
    ...
    if (formData.protocol === "G853") {
      const fieldsG853 = ["triceps", "suprailiac", "abdominal"];
    ...
    if (formData.protocol === "G943") {
      const fieldsG943 = ["subscapularis", "suprailiac", "medialThigh"];
      ...
      if (data.gender === "MAN") {
        ...
        const density = 1.1665 - 0.07063 * Math.log10(sd);
        ...
        const densityWoman = 1.1665 - 0.07063 * Math.log10(sd);
        const densityAverage = (densityMan + densityWoman) / 2;
        const fatPercentage = (4.95 / densityAverage - 4.5) * 100;
    ...
    if (formData.protocol === "DR674") {
      const fieldsDR674 = ["subscapularis", "triceps", "biceps", "suprailiac"];
      ...
      const density = 1.161 - 0.0632 * Math.log10(sd);
    ...
    if (formData.protocol === "L964") {
      const fieldsL964 = ["subscapularis", "triceps", "biceps", "suprailiac"];
    ...
    if (formData.protocol === "T843") {
      const fieldsT843 = ["subscapularis", "triceps", "middleAxillary"];
    ...
    if (formData.protocol === "W88PO") {
      ...
      if (data.gender === "MAN") {
        ...
        // Bicompartimental
        const massagorda =
          0.11077 * abdome - 0.17666 * heightCm + 0.14354 * weight + 51.03301;
        const massamagra = 100 - massagorda;
    ...
    if (formData.protocolCardio === "COO12") {
      const distancia =
        name === "distânciapercorrida"
          ? parseFloat(updatedValue.replace(",", ".")) || 0
          : parseFloat(
              String(formData.distânciapercorrida).replace(",", ".")
            ) || 0;
    ...
    if (formData.protocolCardio === "M1609") {
      const t =
        name === "tempogasto"
          ...
          : String(formData.tempogasto)
              .split(":")
              .map(Number)
              .reduce(
                (acc, curr, index) => acc + curr / Math.pow(60, index),
                0
              ) || 0;
      ...
      const fc =
        name === "frequênciacardíaca"
          ? parseFloat(updatedValue.replace(",", ".")) || 0
          : parseFloat(String(formData.frequênciacardíaca).replace(",", ".")) ||
            0;
    ...
    setFormData((prevState) => ({
      ...
      iac:
        height === 0
          ? 0
          : parseFloat((hip / (height * Math.sqrt(height)) - 18).toFixed(2)),
  ...
  const handleSearchChange = useDebounce(
    async (value: string, isValid: boolean) => {
      if (value.length === 2) {
        setResponseEvaluator(null);
        return;
      }
      ...
      try {
        const response: FindStudentResponse = await request(QueryFindStudent, {
          companyCode: companyCode,
          search: value,
        });
        ...
        const genericError =
          "Ops! Algo deu errado ao localizar avaliador. Tente novamente!";
  ...
  const findReviewQuestions = useCallback(async () => {
    try {
      const response: FindReviewQuestionsResponse = await request(
        QueryFindReviewQuestions
      );
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar questões de avaliações. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar questões de avaliações.", {
        variant: "error",
      });
    }
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Avaliação alterada com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refreshInternal?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao alterar avaliação. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao alterar avaliação. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      } finally {
        setIsLoading(false);
  ...
  const protocolsOptions = [
    ...
    {
      value: "JPW78807",
      label: "Jackson, Pollock e Ward (1978 e 1980) - 7 dobras",
    },
    {
      value: "JPW78803",
      label: "Jackson, Pollock e Ward (1978 e 1980) - 3 dobras",
    },
  ...
  const handleSelectMuiChange = (event: SelectChangeEvent<string>) => {
    ...
    const resetFieldsCardio = ["COO12", "M1609"];
    ...
    if (resetFieldsCardio.includes(value)) {
      ...
      const updatedFormData = fieldsToClear.reduce(
  ...
  const protocolMapping: { [protocol: string]: string[] } = {
    ...
    W88PO: ["abdome", "wristBistyloid", "femurBistyloid", "height", "weight"],
    COO12: ["distânciapercorrida"],
    M1609: ["tempogasto", "frequênciacardíaca", "weight"],
  };
  ...
  const filteredQuestionsCardio =
    responseReviewQuestions?.findReviewQuestions.filter((question) => {
      ...
      const allowedKeys =
        protocolMapping[
          formData.protocolCardio as keyof typeof protocolMapping
        ];
      if (allowedKeys) {
  ...
  const getReviewClassification = (
    ...
    if (type === "RCQ_MAN_50_59") {
      if (imc < 0.9) return "Risco Baixo";
      if (imc >= 0.9 && imc < 0.97) return "Risco Moderado";
    ...
    if (type === "RCQ_MAN_40_49") {
      ...
      if (imc >= 0.96 && imc < 1.0) return "Risco Alto";
      return "Risco Muito Alto";
    }
    ...
    if (type === "RCQ_WOMAN_60_69") {
      ...
      if (imc >= 0.84 && imc < 0.9) return "Risco Alto";
      return "Risco Muito Alto";
    }
    ...
    if (type === "RCQ_WOMAN_40_49") {
      ...
      if (imc >= 0.73 && imc < 0.8) return "Risco Moderado";
      if (imc >= 0.8 && imc < 0.87) return "Risco Alto";
      return "Risco Muito Alto";
    }
  ...
  const getImcRanges = (type: string) => {
    ...
    if (type === "RCQ_MAN_50_59") {
      return [
        { value: 0, position: 2, color: "#4CAF50" },
        { value: 0.9, position: 25.5, color: "#FF9800" }, // Risco Baixo
    ...
    if (type === "RCQ_MAN_40_49") {
      return [
        ...
        { value: 0.96, position: 50.5, color: "#FF5722" }, // Risco Moderado
        { value: 1.0, position: 75.5, color: "#D32F2F" }, // Risco Alto
        { value: 100.0, position: 99, color: "#D32F2F" }, // Risco Muito Alto
      ];
    }
    ...
    if (type === "RCQ_WOMAN_60_69") {
      return [
        ...
        { value: 0.9, position: 75.5, color: "#D32F2F" }, // Risco Alto
        { value: 100.0, position: 99, color: "#D32F2F" }, // Risco Muito Alto
      ];
    }
    ...
    if (type === "RCQ_WOMAN_40_49") {
      return [
        ...
        { value: 0.8, position: 50.5, color: "#FF5722" }, // Risco Moderado
  ...
  const getImcPosition = (imc: number, type: string) => {
    ...
    for (let i = 1; i < imcRanges.length; i++) {
      if (imc < imcRanges[i].value) {
        const percentDiff =
          (imc - previous.value) / (imcRanges[i].value - previous.value);
        const position =
          previous.position +
          percentDiff * (imcRanges[i].position - previous.position);
  ...
  const rcqColorWoman60and69 = getImcColor(
    Number(formData.rcq),
    "RCQ_WOMAN_60_69"
  );
  const rcqColorWoman50and59 = getImcColor(
    Number(formData.rcq),
    "RCQ_WOMAN_50_59"
  );
  const rcqColorWoman40and49 = getImcColor(
    Number(formData.rcq),
    "RCQ_WOMAN_40_49"
  );
  const rcqColorWoman30and39 = getImcColor(
    Number(formData.rcq),
    "RCQ_WOMAN_30_39"
  );
