...
export const useStudentAlterForm = ({
  ...
  const [responseTransactions, setResponseTransactions] =
    useState<FindTransactionsResponse | null>();
  const [responseStudentPlans, setResponseStudentPlans] =
    useState<FindStudentPlansResponse | null>();
  const [responseReviews, setResponseReviews] =
    useState<FindReviewsResponse | null>();
  ...
  const [studentPlanDetail, setStudentPlanDetail] =
    useState<StudentPlanData | null>();
  ...
  const filteredTransactions = (
    responseTransactions?.findTransactions || []
  ).filter((transaction) => {
    ...
    const matchesSearch =
      ...
      (transaction.transactionCode &&
        extractedProfileCode &&
        transaction.transactionCode.toString() === extractedProfileCode);

    const matchesStatus =
      selectedStatus.length === 0 ||
      selectedStatus.includes(transaction.paymentStatus);
  ...
  const endIndexReviews = Math.min(
  ...
  const findTransactions = useCallback(async () => {
    try {
      const response: FindTransactionsResponse = await request(
        QueryFindTransactions,
        { recipientCode: Number(data.profileCode) }
      );
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar transações. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar as transações do aluno.", {
        variant: "error",
      });
    }
  ...
  const findStudentPlans = useCallback(async () => {
    try {
      const response: FindStudentPlansResponse = await request(
        QueryFindStudentPlans,
        { companyCode: companyCode, studentCode: Number(data.profileCode) }
      );
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar planos associados ao aluno. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar planos associados ao aluno.", {
        variant: "error",
      });
    }
  ...
  const findReviews = useCallback(async () => {
    try {
      const response: FindReviewsResponse = await request(QueryFindReviews, {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar avaliações associados ao aluno. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar("Erro ao buscar avaliações associados ao aluno.", {
        variant: "error",
      });
    }
  }, [attemptCount, companyCode, data.profileCode, enqueueSnackbar, request]);
  ...
  const [drawerType, setDrawerType] = useState<
    AdminGymDetailsProps[keyof AdminGymDetailsProps] | null
  >(null);
  ...
  const openDrawer = (
    type: AdminGymDetailsProps[keyof AdminGymDetailsProps],
  ...
  const handleCrop = async () => {
    if (cropperRef.current) {
      ...
      setIsLoading(true);
      try {
        const variables = SendCroppedImageVariables(
          data.profileCode,
          "STUDENT",
          croppedDataURL
        );
        await request(MutationSendCroppedImage, variables);
        enqueueSnackbar("Foto da matricula alterada com sucesso!", {
          variant: "success",
        });
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao alterar foto da matricula. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao alterar foto da matricula. Tente novamente!";
  ...
  const handleDeleteImage = async () => {
    try {
      ...
      enqueueSnackbar("Foto da matricula excluída com sucesso!", {
        variant: "success",
      });
      refresh?.();
    } catch (error: unknown) {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir foto da matricula. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir foto da matricula. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const handleOpenMore = (
    ...
    setAnchorEls((prev) => ({
      ...prev,
      [transactionCode]: event.currentTarget,
    }));
  };
  ...
  const handleMoreDetails = (transactionCode: string) => {
    const transactionDetail = responseTransactions?.findTransactions?.find(
  ...
  const handleMoreDetailsReview = (reviewCode: string) => {
    const reviewDetail = responseReviews?.findReviews?.find(
  ...
  const handleStudentPlansMoreDetails = (studentPlanCode: string) => {
    const studentPlanDetail = responseStudentPlans?.findStudentPlans?.find(
      (plan) => plan.studentPlanCode === studentPlanCode
    );
  ...
  const handleCancelTransaction = async (transactionCode: string) => {
    try {
      ...
      enqueueSnackbar("Transação cancelada com sucesso", {
        variant: "success",
      });
      refreshInternal("findTransactions");
    } catch (error: unknown) {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao cancelar transação. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao cancelar transação. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const handleReprocessTransaction = async (transactionCode: string) => {
    try {
      ...
      enqueueSnackbar("Transação reprocessada com sucesso", {
        variant: "success",
      });
      refreshInternal("findTransactions");
    } catch (error: unknown) {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao reprocessar transação. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao reprocessar transação. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const handleDeleteTransaction = async (transactionCode: string) => {
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir transação. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir transação. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const handleDeleteReview = async (reviewCode: string) => {
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao excluir avaliação. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao excluir avaliação. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
  ...
  const filterStatusOptions = [
    {
      value: "noSelect",
      label: "Selecione um ou mais status",
      isDisabled: true,
    },
  ...
  const handleStatusPlan = async (
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao ativar/desativar plano do aluno. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao ativar/desativar plano do aluno. Tente novamente!";
      const errorMessage = GetErrorMessage(error, genericError);
      enqueueSnackbar(errorMessage, { variant: "error" });
    }
  };
