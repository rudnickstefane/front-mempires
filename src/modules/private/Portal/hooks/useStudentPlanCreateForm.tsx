  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        const paymentDay = formData.paymentDay
          ? new Date(
              Date.UTC(
                ...
                0,
                0,
                0 // Hora, minuto, segundo, milissegundo zerados
              )
            )
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao associar plano com aluno. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao associar plano com aluno. Tente novamente!";
