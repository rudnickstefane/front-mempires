...
export const usePayTransactionForm = ({
  ...
  const [preferenceCode, setPreferenceCode] = useState<PreferenceData | null>(
    null
  );
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const response: PreferenceData = await request(
        MutationCreatePaymentPreference,
        variables
      );
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao seguir com pagamento. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      const genericError =
        "Ops! Algo deu errado ao seguir com o pagamento. Tente novamente!";
