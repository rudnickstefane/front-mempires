...
export const useContributorEditForm = ({
  closeDrawer,
  enqueueSnackbar,
  data,
  refresh,
  ...
  const [responseGroupsOptions, setResponseGroupsOptions] = useState<
  ...
  const handleFocus = (
    field: "position" | "gender" | "stateMarital" | "groupCode" | "modalities",
    value: boolean
  ) => {
  ...
  const [formData, setFormData] = useState<ReviewsProps["formData"]>({
    ...
    emergencyContact: data.contact[0].emergencyContact,
    emergencyPhone: data.contact[0].emergencyPhone,
    responsible: "",
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Alterações realizadas com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao atualizar colaborador. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao atualizar colaborador. Tente novamente!";
  ...
  const findGroups = useCallback(async () => {
    try {
      const response: FindGroupsResponse = await request(QueryFindGroups, {
      ...
      setResponseGroupsOptions([
        {
          value: "noSelect",
          label: "Selecione um grupo de acesso",
          isDisabled: true,
        },
        ...groups,
      ]);
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar grupos de acesso. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar(
        "Erro ao buscar grupos de acesso cadastrados em sua academia.",
        { variant: "error" }
      );
    }
