...
export const useCompanyGymManagement = ({ data, refresh }: ProfileGymManagementProps) => {
    ...
    const handleCrop = async () => {
        if (cropperRef.current) {
            ...
            setIsLoading(true);
            try {
                const variables = SendCroppedImageVariables(companyCode, 'COMPANY', croppedDataURL);
    ...
    const handleDeleteImage = async () => {
        try {
            const variables = SendCroppedImageVariables(companyCode, 'COMPANY', 'data:(.+);base64,', 'DELETE');
            await request(MutationSendCroppedImage, variables);
