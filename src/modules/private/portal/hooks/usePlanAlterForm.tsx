...
export const usePlanAlterForm = ({
  ...
  const [responseModalitiesOptions, setResponseModalitiesOptions] = useState<
    Array<{
      value: string;
      label: string;
      amount: string;
  ...
  >([]);
  const [responseCustomServiceOptions, setResponseCustomServiceOptions] =
    useState<
      Array<{
        value: string;
        label: string;
    ...
    >([]);
  const [responsePeriodicityOptions, setResponsePeriodicityOptions] = useState<
    Array<{ value: string; label: string; isDisabled?: boolean }>
  >([]);
  ...
  const [formData, setFormData] = useState<PlanCreateProps["formData"]>({
    ...
    periodicity: data.findPlans.periodicities.map(
      (item: { periodicity: any }) => item.periodicity
    ),
    periodicityDetails: data.findPlans.periodicities.map(
      (item: {
        ...
        const totalModalityAmount = data.findPlans.modalities
          .map((modality: { amount: string | number | undefined }) =>
            parseToFloat(modality.amount)
          )
        ...
        const totalCustomServiceAmount = data.findPlans.customServices
          .map((service: { amount: string | number | undefined }) =>
            parseToFloat(service.amount)
          )
          .reduce((sum: any, value: any) => sum + value, 0);

        // Subtrai os valores do amount da periodicity
        const adjustedAmount =
          parseToFloat(item.amount) -
          (totalModalityAmount + totalCustomServiceAmount);
    ...
    endHours: data?.findPlans.endDate
      ...
      : "",
    modalities: data.findPlans.modalities.map(
      (item: { categoryCode: any }) => item.categoryCode
    ),
    customService: data.findPlans.customServices.map(
      (item: { categoryCode: any }) => item.categoryCode
    ),
    chargeRegistration:
      data.findPlans.typeCharge === "PLAN"
        ? data.findPlans.periodicities[0].charge
        : "",
    ...
    isPercentage:
      data.findPlans.calculationFees === "PERCENTAGE" ? true : false,
  ...
  const accessOptions = [
    {
      value: "noSelect",
      label: "Selecione um tipo de acesso",
      isDisabled: true,
    },
  ...
  const [selectedChargeValue, setSelectedChargeValue] = useState(
    data.findPlans.typeCharge === "PLAN" ? "isPlan" : "isPeriodicity"
  );
  const [selectedHoursValue, setSelectedHoursValue] = useState(
    formData.isAllHours ? "isAllHours" : "isCustomHours"
  );
  const [selectedFeesFrequencyValue, setSelectedFeesFrequencyValue] = useState(
    data.findPlans.feesFrequency === "UNIQUE" ? "isUnique" : "isDaily"
  );
  const [selectedCalculationBaseValue, setSelectedCalculationBaseValue] =
    useState(
      data.findPlans.calculationBase === "VALUE" ? "isValue" : "isPercentage"
    );
  ...
  const handleTextFieldChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (name.startsWith("observation-")) {
      ...
      if (value.length <= 300) {
        setFormData((prevState) => {
          const updatedPeriodicityDetails = prevState.periodicityDetails.map(
            (detail) => {
  ...
  function handleRemovePeriodicity(periodicityToRemove: string) {
    setFormData((prevState) => ({
      ...prevState,
      periodicity: prevState.periodicity.filter(
        (periodicity) => periodicity !== periodicityToRemove
      ),
      periodicityDetails: prevState.periodicityDetails.filter(
        (detail) => detail.value !== periodicityToRemove
      ),
    }));
  }
  ...
  const transformFormDataToPlanSummary = (
    formData: any
  ): SetPlanDataSummary => {
    const {
      name,
    ...
    const mappedModalities = modalities
      .map((modalityId: string) => {
        const modality = responseModalitiesOptions.find(
          (option) => option.value === modalityId
        );
        return modality
          ? {
      ...
      .filter(
        (
          modality: any
        ): modality is { value: string; label: string; amount: string } =>
          modality !== null
      );

    const mappedCustomService = customService
      .map((customServiceId: string) => {
        const customService = responseCustomServiceOptions.find(
          (option) => option.value === customServiceId
        );
    ...
    const calculateTotal = (items: { amount: string }[]) =>
      items.reduce((total, item) => {
        ...
        const amount = parseFloat(
          item.amount.replace(/[R$.\s]/g, "").replace(",", ".")
        );
    ...
    const periodicities = periodicityDetails.map((detail: any) => {
      ...
      const totalValue =
        parseFloat(
          detail.amount?.replace(/[R$.\s]/g, "").replace(",", ".") || "0"
        ) +
        totalModalities +
        totalCustomService;
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormPlanAlter(
      formData,
      activeStep,
      startTime,
      endTime,
  ...
  const findCategories = useCallback(async () => {
    try {
      const response: FindCategoriesResponse = await request(
        QueryFindCategories,
        { companyCode: companyCode }
      );
      ...
      if (modalities.length === 0) {
        ...
        setResponseModalitiesOptions([
          {
            value: "noSelect",
            label: "Nenhuma modalidade disponível",
            amount: "",
            isDisabled: true,
          },
        ]);
        ...
        setResponseModalitiesOptions([
          {
            value: "noSelect",
            label: "Selecione uma ou mais modalidades",
            amount: "",
            isDisabled: true,
          },
          ...modalities,
      ...
      const customService = response.findCategories.customService.map(
        (modality) => ({
          value: modality.categoryCode,
          label: modality.name,
          amount: modality.amount,
      ...
      if (customService.length === 0) {
        setResponseCustomServiceOptions([
          {
            value: "noSelect",
            label: "Nenhum serviço personalizado disponível",
            amount: "",
            isDisabled: true,
          },
        ]);
        ...
        setResponseCustomServiceOptions([
          {
            value: "noSelect",
            label: "Selecione um ou mais serviços personalizados",
            amount: "",
            isDisabled: true,
          },
          ...customService,
      ...
      setResponseModalitiesOptions([
        {
          value: "noSelect",
          label: "Ocorreu um erro ao buscar suas modalidades.",
          amount: "",
          isDisabled: true,
        },
      ]);
      ...
      setResponseCustomServiceOptions([
        {
          value: "noSelect",
          label: "Ocorreu um erro ao buscar seus serviços personalizados.",
          amount: "",
          isDisabled: true,
        },
      ]);
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar modalidades e serviços personalizados. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }

      enqueueSnackbar(
        "Erro ao buscar modalidades cadastradas em sua academia.",
        { variant: "error" }
      );
      enqueueSnackbar(
        "Erro ao buscar serviços personalizados cadastradas em sua academia.",
        { variant: "error" }
      );
    }
    ...
    if (!calledRef.current) {
      ...
      const fetchData = async () => {
        ...
        const periodicity = Array.from({ length: 24 }, (_, index) => {
          ...
          return {
            value,
            label: `${GeneratePeriodLabel(
              index + 1
            )} - ${GeneratePeriodDescription(index + 1)}`,
        ...
        setResponsePeriodicityOptions([
          {
            value: "noSelect",
            label: "Selecione uma ou mais periodicidades",
            isDisabled: true,
          },
          ...periodicity,
        ]);
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao atualizar plano. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao atualizar o plano. Tente novamente!";
