...
import {
  ...
  FormatText,
  GetErrorMessage,
  GroupsUpsertVariables,
  ValidateFormRegisterGroup,
} from "../../../common/utils";
...
export const useGroupEditForm = ({
  ...
  const [formData, setFormData] = useState<ReviewsProps["formData"]>({
    ...
    permissionProfileCompany: data.permissions.find(
      (p: { path: string }) => p.path === "Company"
    )?.permission,
    permissionMenuAdmin: data.permissions.find(
      (p: { path: string }) => p.path === "Admin"
    )?.permission,
    permissionMenuFinance: data.permissions.find(
      (p: { path: string }) => p.path === "Finance"
    )?.permission,
    permissionMenuIntegration: data.permissions.find(
      (p: { path: string }) => p.path === "Integration"
    )?.permission,
    permissionMenuDashboard: data.permissions.find(
      (p: { path: string }) => p.path === "Dashboard"
    )?.permission,
    permissionMenuPrograms: data.permissions.find(
      (p: { path: string }) => p.path === "Programs"
    )?.permission,
  });
      ...
      try {
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao atualizar grupo. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao atualizar grupo. Tente novamente!";
