...
import { MutationVisitRegister, QueryFindStudent } from "../pages/home/graphql";
import { FindStudentResponse } from "../pages/home/types";
import { VisitRegisterVariables } from "../pages/home/utils";

export const useVisitAlterForm = ({
  closeDrawer,
  enqueueSnackbar,
  data,
  refresh,
  ...
  const handleFocus = (
    field: "referralSource" | "modalities",
    value: boolean
  ) => {
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao buscar modalidades. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar(
  ...
  const [formData, setFormData] = useState<VisitRegisterProps["formData"]>({
    ...
    indicationSearch: data.nameIndication,
    modalities: data.categories?.map((item: { categoryCode: string }) =>
  ...
  const validateForm = () => {
    ...
    const indicationCode = responseStudent?.findStudent.profileCode
      ? responseStudent.findStudent.profileCode
      : data.indicationCode;

    newErrors = ValidateFormVisitRegister(
      formData,
      activeStep,
      indicationCode,
      isIndication
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Visitante cadastrado com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao cadastrar visitante. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao cadastrar visitante. Tente novamente!";
        const errorMessage = GetErrorMessage(error, genericError);
        enqueueSnackbar(errorMessage, { variant: "error" });
      } finally {
        setIsLoading(false);
