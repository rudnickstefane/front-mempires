...
import {
  ClassCreateVariables,
  FormatName,
  GeneratePeriodDescription,
  GeneratePeriodLabel,
  GetErrorMessage,
  ValidateFormClassCreate,
} from "../../../common/utils";
...
export const useClassAlterForm = ({
  ...
  const [formData, setFormData] = useState<ClassCreateProps["formData"]>({
    ...
    endHours: data?.findClasses.endDate
      ...
      : "",
    modalities: data.findClasses?.modalities?.map(
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormClassCreate(
      formData,
      activeStep,
      startTime,
      endTime,
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        enqueueSnackbar("Turma atualizada com sucesso!", {
          variant: "success",
        });
        closeDrawer();
        refresh?.();
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao atualizar turma. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao atualizar turma. Tente novamente!";
