...
export const useEditStudentForm = ({
  ...
  const handleAddContact = () => {
    if (validateForm()) {
      ...
      setContacts((prevContacts: any[]) => {
        const mainContact = prevContacts.find(
    ...
    try {
      ...
      if (attemptCount >= 5) {
        return enqueueSnackbar(
          "Erro ao apagar contato. Entre em contato com nosso suporte.",
          { variant: "error" }
        );
      }
      enqueueSnackbar(
        "Ops! Algo deu errado ao apagar o contato. Tente novamente!",
        { variant: "error" }
      );
    }
  };
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormEditStudent(
      formData,
      activeStep,
      indicationCode,
      isIndication
  ...
  const handleFinish = async () => {
    if (contacts) {
      ...
      try {
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao atualizar seus dados. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }
