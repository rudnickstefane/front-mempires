...
import "../../../../../input.css";
import { useGymData } from "../../pages/home/hooks/dbHome";
...
export function ManagementGymHeader({
  setActiveComponent,
}: ManagementGymHeaderProps) {
  const [activeButton, setActiveButton] = useState<GymManagementType>("Home");

  const { data: routes, loading } = useGymData<Route[]>(
    "http://localhost:5000/RoutesManagement"
  );
        ...
        {loading
          ...
          : routes?.map((route) => (
              <Button
                ...
                startIcon={
                  <img
                    src={RoutesIcons[route.button as keyof typeof RoutesIcons]}
                    style={{ width: "1.25rem" }}
                    alt={route.name}
                  />
                }
