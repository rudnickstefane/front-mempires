...
import {
  CreateAccessVariables,
  ValidateFormCreateAccess,
} from "../../../utils";

export const useSignUpSupplierForm = ({
  enqueueSnackbar,
}: {
  enqueueSnackbar: (
    message: string,
  ...
  const validateForm = () => {
    ...
    newErrors = ValidateFormCreateAccess(formData, activeStep, "SUP");
    setErrors(newErrors);
  ...
  const handleFinish = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        const response: CreateAccessResponse = await request(
          MutationCreateAccess,
          variables
        );
        ...
        const genericError =
          "Ops! Algo deu errado ao criar sua conta. Tente novamente!";
