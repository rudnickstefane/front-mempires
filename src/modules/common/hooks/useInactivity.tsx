  ...
  const handleAccess = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        const response: CreateTokenResponse = await request(
          MutationCreateToken,
          variables
        );
        ...
        localStorage.setItem(
          "@iflexfit:role",
          response.createToken.user.profiles[0].role
        );
        localStorage.setItem(
          "@iflexfit:status",
          response.createToken.user.profiles[0].status
        );
        localStorage.setItem(
          "@iflexfit:email",
          response.createToken.user.email
        );
        localStorage.setItem("@iflexfit:name", response.createToken.user.name);
        localStorage.setItem(
          "@iflexfit:profileCode",
          response.createToken.user.profileCode
        );
        localStorage.setItem(
          "@iflexfit:companyCode",
          companyCode ?? assignment
        );

        setInactivityLogIn(false);
        navigate("/portal");
        enqueueSnackbar("Olá, seja bem-vindo novamente!", {
          variant: "success",
        });
      } catch (error: unknown) {
        ...
        const genericError =
          "Ops! Algo deu errado ao autenticar-se. Tente novamente!";
