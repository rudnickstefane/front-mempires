  ...
  const handleRecovery = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        navigate("/entrar");
        enqueueSnackbar("Acesso recuperado com sucesso!", {
          variant: "success",
        });
      } catch (error: unknown) {
        ...
        if (attemptCount >= 5) {
          return enqueueSnackbar(
            "Erro ao recuperar acesso. Entre em contato com nosso suporte.",
            { variant: "error" }
          );
        }

        const genericError =
          "Ops! Algo deu errado ao recuperar seu acesso. Tente novamente!";
    ...
    if (!calledRef.current) {
      ...
      const checkCredentials = async () => {
        ...
        try {
          ...
          if (attemptCount >= 5) {
            return enqueueSnackbar(
              "Erro ao validar link de recuperação. Entre em contato com nosso suporte.",
              { variant: "error" }
            );
          }

          const genericError =
            "Ops! Algo deu errado ao validar o link de recuperação. Tente novamente!";
          const errorMessage = GetErrorMessage(error, genericError);
          enqueueSnackbar(errorMessage, { variant: "error" });
        }
      };
