...
export const useGymConfigFormStepFive = (formData: StepFiveProps['formData'], setFormData: StepFiveProps['setFormData']) => {
    ...
    const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>, itemName: string, itemType: 'service' | 'segment') => {
        ...
        if (event.target.checked) {
            setFormData({ ...formData, [itemType === 'service' ? 'selectedServices' : 'selectedSegments']: [...selectedItems, itemName] });
        } else {
            setFormData({ ...formData, [itemType === 'service' ? 'selectedServices' : 'selectedSegments']: selectedItems.filter(item => item !== itemName) });
        }
    };
